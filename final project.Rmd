---
title: "Project title"
author: "Wensha Sun, TUT0207, and group number"
...
#define Morning, afternoon, evening, night
```{r}
mod_break_and_enters <- break_and_enters %>% filter(!is.na(occurrenceyear)) %>% filter(!is.na(occurrencedayofweek)) %>% filter(!is.na(occurrencemonth)) %>% # group_by(occurrenceyear, occurrencemonth)
mutate(occurrencehourinday = ifelse(occurrencehour >= 20 | occurrencehour < 6, "Night", ifelse(occurrencehour >= 6 & occurrencehour < 12, "Morning", ifelse(occurrencehour >= 12 & occurrencehour <17, "Afternoon", "Evening"))))
```

#classified by type, we notice that (obviously) house endure the most break and enters occurrence. 
#so next step we want to know whether mean danger = mean other
#hypothesis test
```{r}
mod_break_and_enters %>% group_by(premisetype) %>% summarise(n=n())
ggplot(data = mod_break_and_enters, aes(x = premisetype)) + geom_bar() + coord_flip()
```